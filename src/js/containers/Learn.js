  ...
  render() {
    const header = (
      <div>
        {this.props.language === "EN" ? (
          <Paper className={css(styles.headerPaper)}>
            Reasons why the new Icelandic constitution must be enacted
          </Paper>
        ) : null}
        {/*Change this.  It still says "TEN" reasons in Icelandic. */}
        {this.props.language === "IS" ? (
          <Paper className={css(styles.headerPaper)}>
            Tíu ástæður fyrir því að nauðsynlegt er að nýja stjórnarskráin taki
            gildi
          </Paper>
        ) : null}
      </div>
    ...
    return (
      <div>
        ...
        <div className={css(styles.headerPaper)}>
          {wordForArticles[this.props.language]}
        </div>
        {articles.map((arti, i) => (
          <LearnArticle article={arti} key={"article" + i} />
        ))}
      </div>
    );
  }
